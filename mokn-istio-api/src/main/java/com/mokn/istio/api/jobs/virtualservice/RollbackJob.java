...
public class RollbackJob {
    ...
    public void run(){
        ...
        condition.setVersionStatus(VersionStatusEnum.ROLLBACK_ING.getValue());
        ...
        for(IstioRouteVersion version:versions){
            ...
            for(IstioRouteVersionItem item:version.getItems()){
                ...
                Deployment deployment=deploymentService.getDeployment(item.getNamespace(),item.getNameHost()+"-"+item.getSubsetOld());
                if(deployment==null || deployment.getMetadata()==null
                        ...
                        || !deployment.getMetadata().getLabels().getVersion().equals(item.getSubsetOld())){
                    isRun=false;
                    break;
                }
            }
            ...
            if(isRun){
                ...
                for(IstioRouteVersionItem item:version.getItems()){
                    ...
                    String applyJson=template.replace("{{.HOST}}",item.getNameHost())
                            ...
                            .replace("{{.ISTIO_VERSION}}",item.getSubsetOld())
                            .replace("{{.ISTIO_VERSION_TYPE}}",resourceType.getValue())
                            .replace("{{.SUBSET}}",item.getSubsetOld());
                ...
                versionService.action(version.getSysno(),VersionStatusEnum.ROLLBACK_SUCCESS);
            }
