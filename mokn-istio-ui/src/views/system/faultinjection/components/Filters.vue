   ...
   <!--查询过滤条件部分-->
    <el-col :span="24" class="toolbar" style="padding-top:10px;">
        <el-form :inline="true" :model="filters" ref="filtersForm" size="small">
          ...
          <el-form-item>
            <el-button type="primary" @click="getVirtualServices" icon="el-icon-search" >查询</el-button>
          </el-form-item>
          <el-form-item>
            <el-button @click="resetFilters({virtualservices:''})" icon="el-icon-zoom-in">清空</el-button>
          </el-form-item>
        </el-form>
        </el-col>
  </section>
...
<script>
  ...
  import {namespaceList,virtualServiceFailSet} from '@/service/api';
  const {mapGetters, mapActions} = createNamespacedHelpers('system/virtualservice')
  export default{
    name: 'FaultInjectionFilters',
    data () {
      return {
        namespaces:[]
      }
    ...
    methods: {
      //导入store actions
      ...mapActions(['getVirtualServices', 'resetFilters']),
      getNamespaces() {
              let param={
              };
