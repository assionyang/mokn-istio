<template>
	<section>
	    <!--表格部分-->
		<el-col :span="24">
    		<el-table :data="configmaps" v-loading="listLoading"  style="width: 100%;"  size="mini" header-row-style="background:#F2F6FC">
    		<template>
				...
				<el-table-column label="数据" align="center">
					<template slot-scope="scope">
						...
						<el-tooltip placement="top">
  					        <div slot="content">
								  <span v-for="o in scope.row.dataList" :key="o">
							           {{o}}<br/>
						          </span>
							</div>
...
import {configmapDelete} from '@/service/api';
const {mapGetters,mapActions}=createNamespacedHelpers('system/configmap');
	export default{
		...
		computed:{
			//导入store的getters属性
			...mapGetters(['configmaps','listLoading'])
		},
		//当前方法
		methods:{
			//导入actions方法
            ...
            delSubmit(row){
                ...
                configmapDelete(param).then((data)=>{
