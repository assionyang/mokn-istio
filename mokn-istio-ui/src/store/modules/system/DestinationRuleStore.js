...
import {destinationruleList} from '@/service/api';
//状态
const  state={
  destinationrules:[],//
  //总记录数
...
const getters={
  destinationrules:state=>state.destinationrules,
...
const actions={
  ...
  getDestinationRules({commit,state}) {
    let param={
      namespace:state.filters.namespace
    };
    commit('listLoading',true);
    destinationruleList(param).then((res)=>{
...
const mutations={
  loadList(state,param){
    ...
    state.destinationrules=param.rows;
  },
  //表格载入状态变更
